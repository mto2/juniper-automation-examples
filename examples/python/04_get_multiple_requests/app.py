...
for device in devices:
    with Device(host=device, user='ansible', password='hol-leap!bru6PSOO') as dev:
        try:
            ...
            pass
        try:
            show_chassis_hardware = dev.rpc.get_chassis_inventory({'format':'json'})
            write_to_file(device, show_chassis_hardware)
        except:
            pass
        try:
            show_config = dev.rpc.get_config(options={'format':'json'})
            write_to_file(device, show_config)
        except:
            pass
